...
public class EvernoteNote implements NoteManage<Notebook, Note> {
    ...
    public String createNote(String guid, String note_title, String note_body) {
        ...
        } catch (EDAMUserException | EDAMSystemException | EDAMNotFoundException | TException e1) {
            e1.printStackTrace();
        }
    ...
    private static NoteStoreClient createNoteStoreClient(String access_token) {
        EvernoteAuth evernoteAuth = new EvernoteAuth(EvernoteService.SANDBOX, access_token);
            ...
            do {
                ...
                for (Note note : notes.getNotes()) {
                    ...
                    try {
                        ...
                        System.out.println("Note links : " + String.format("https://%s/shard/%s/nl/%s/%s/",
                                evernoteHost, user.getShardId(), user.getId(), note.getGuid()));
                        System.out.println("Note public links : " + String.format("https://%s/shard/%s/sh/%s/%s/",
                                evernoteHost, user.getShardId(), note.getGuid(), shardkey));
