...
public class TrelloUtils<T> {
    ...
    public static void main(String[] args) {
        ...
        if (create) {
            ...
            for (Map<String, Object> board : boards) {
                ...
                logger.info(String.format("name:%s # id:%s # closed:%b # idOrganization:%s # pinned:%s", name, id,
                        closed, idOrganization, pinned));

                if (StringUtils.equals(name, "hiwork")) {
                    String resp_webhook = createWebhook(
                            "http://113.143.148.112:808/jersey/webhook/trello/board/callback", "board webhook", id);
    ...
    public static String getAuthUrl() {
        ...
        String req_url = String.format("%s?key=%s&name=%s&expiration=%s&response_type=%s&scope=read,write", url, key,
                name, expires, resp_type);
        logger.info(req_url);
        return req_url;
    }
    ...
    public static String getOauthUrl() {
        ...
        OAuthService service = new ServiceBuilder().provider(TrelloApi.class).apiKey(key).apiSecret(secret)
        ...
        String req_url = String.format("%s&name=%s&scope=%s&expiration=%s&oauth_callback=%s", auth_url, name,
                URLEncoder.encode(scope), expires, URLEncoder.encode(callback));
     ...
     * @return
     */
    public static String getAccessToken(String Oauth_token, String oauth_verifier) {
        OAuthService service = new ServiceBuilder().provider(TrelloApi.class).debug().apiKey(key).apiSecret(secret)
                .build();
        Token request_token = service.getRequestToken();
        Token access_token = service.getAccessToken(request_token, new Verifier(oauth_verifier));
     ...
     * @return
     */
    public static String createWebhook(String callback, String desc, String board_id) {
        ...
        try {
            ...
            StringEntity entity = new StringEntity(req_data, ContentType.APPLICATION_JSON);
     ...
     * @return
     */
    public static String updateWebhookWithModelId(String webhook_id, String model_id) {
        String req_url = String.format("https://trello.com/1/webhooks/%s/idModel", webhook_id);
        CloseableHttpClient hpclient = NETUtils.getHttpClient(true);
        HttpPut hpput = new HttpPut(req_url);
        HttpEntity entity = EntityBuilder.create().setParameters(new BasicNameValuePair("value", model_id),
                new BasicNameValuePair("key", key), new BasicNameValuePair("token", my_access_token)).build();
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                logger.info(String.format("webhook update failed : webhook id[%s] is not exist: ", webhook_id));
            }
        } catch (ClientProtocolException e) {
            e.printStackTrace();
        } catch (IOException e) {
    ...
    public static String deleteWebhook(String webhook_id) {
        ...
        String req_url = String.format("https://trello.com/1/webhooks/%s?key=%s&token=%s", webhook_id, key,
                my_access_token);
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                logger.info(String.format("webhook delete error : webhook id[%s] is not exist: ", webhook_id));
            }
        } catch (ClientProtocolException e) {
            e.printStackTrace();
        } catch (IOException e) {
    ...
    public static String getMembers(String username) {
        ...
        String req_url = String.format("%s/%s?boards=all&key=%s&token=%s", url, username, key, my_access_token);
        String resp_body = NETUtils.request(HttpGet.METHOD_NAME, req_url, "");
        logger.info(resp_body);
        return resp_body;
    }
    ...
    public static String getNotifications() {
        ...
        HttpEntity entity = EntityBuilder.create()
                .setParameters(new BasicNameValuePair("key", key), new BasicNameValuePair("token", my_access_token))
                .build();
