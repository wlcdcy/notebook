...
public class EvernoteAPIOauth2 implements APIOauth2 {
    ...
    public String getAccessToken(String auth_code) {
        ...
        String param = String.format(
                "oauth_consumer_key=%s&oauth_token=%s&oauth_verifier=%s&oauth_nonce=%s&oauth_signature=%s&oauth_signature_method=%s&oauth_timestamp%s&oauth_version=%s",
                consumer_key, oauth_token, oauth_verifier, Long.toHexString(random.nextLong()), consumer_secret,
                "PLAINTEXT", Long.toString(System.currentTimeMillis() / 1000), "1.0");
    ...
    private String getTemporaryToken(String redirect_uri) {
        String url = "/oauth";
        String param = String.format(
                "oauth_callback=%s&oauth_consumer_key=%s&oauth_nonce=%s&oauth_signature=%s&oauth_signature_method=%s&oauth_timestamp%s&oauth_version=%s",
                URLEncoder.encode(redirect_uri), consumer_key, Long.toHexString(random.nextLong()), consumer_secret,
                "PLAINTEXT", Long.toString(System.currentTimeMillis() / 1000), "1.0");
        String req_url = oauth_host + url + "?" + param;
