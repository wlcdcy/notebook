...
public class WeixinListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        String queueSize = sce.getServletContext().getInitParameter(MESSAGE_QUEUE_SIZE);
        if (StringUtils.isNotEmpty(queueSize)) {
            try {
                WeixinConfig.getInstance().setMessageQueueSize(Integer.valueOf(queueSize));
            } catch (NumberFormatException e) {
                e.printStackTrace();
                throw new ConfigurationException("valid custom Weixin Configuration [queueSize],it value is int type");
            }
        }
        String reply_model = sce.getServletContext().getInitParameter(MESSAGE_REPLY_SYNC);
        if (StringUtils.isNotEmpty(reply_model)) {
            if (StringUtils.equals("true", reply_model)) {
                ...
                ws = new WeixinMessageTask(WeixinConfig.getInstance().getBqueue(), poolSize);
